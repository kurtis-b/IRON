...
class AIEGeLU(AIEOperatorBase):
    ...
    def __init__(
        ...
        # Enforce ShimDMA limits for GeLU (uses 1 input per core)
    ...
    def get_artifacts(self, prefix="gelu_"):
        ...
        mlir_artifact = PythonGeneratedMLIRArtifact.new(
            ...
            import_path=self.base_dir / "example" / "gelu" / "gelu.py",
            callback_fn="my_gelu",
            callback_args=[
        ...
        xclbin_artifact = XclbinArtifact.new(
            ...
            depends=[
                ...
                KernelObjectArtifact.new(
                    f"gelu.o", depends=[SourceArtifact.new("aie_kernels/aie2p/gelu.cc")]
                ),
            ],
        )
    ...
    def set_up(self):
        ...
        self.add_kernel(
            "gelu", xclbin_artifact, xclbin_artifact.kernel_name, insts_artifact
        )
    ...
    def forward(self, x):
        ...
        if not applicable:
            raise AIEOperatorConstraintError("AIEGeLU: incompatible tensor shape(s)")
    ...
    def _execute_aie_operation(self, x, y=None):
        """Execute GeLU operation on AIE hardware"""
        # x is [batch, size]
        batch = x.shape[0] if x.dim() > 1 else 1
