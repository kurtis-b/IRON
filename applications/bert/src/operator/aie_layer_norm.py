...
class AIELayerNorm(AIEOperatorBase):
    """AIE-accelerated Layer Normalization layer"""
    ...
    def __init__(
        ...
        # Enforce ShimDMA limits for weighted Layer Norm (uses 2 inputs per core)
    ...
    def set_up(self):
        ...
        file_name_base = f"weighted_layer_{self.num_columns}c_{self.num_channels}ch_{self.size}_{self.tile_size}t"
                ...
                KernelArchiveArtifact.new(
                    ...
                    depends=[
                        KernelObjectArtifact.new(
                            f"layer_norm.o",
                            depends=[
                                SourceArtifact.new("aie_kernels/aie2p/layer_norm.cc")
                            ],
                        ),
                        KernelObjectArtifact.new(
                            "mul.o",
        ...
        self.add_kernel(
            "layer_norm", xclbin_artifact, xclbin_artifact.kernel_name, insts_artifact
        )
        self.add_to_runlist("layer_norm", "input1", "input2", "output")
    ...
    def forward(self, x, y=None):
        ...
        if not applicable:
            raise AIEOperatorConstraintError(
                "AIELayerNorm: incompatible tensor shape(s)"
            )
    ...
    def _execute_aie_operation(self, x, y=None):
        """Execute Layer normalization on AIE hardware"""
        # x, y are [batch, size]
        batch = x.shape[0] if x.dim() > 1 else 1
