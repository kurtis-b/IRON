...
class BertFeedForward(nn.Module):
    def __init__(self, config, seq_len=512):
        super().__init__()
        self.config = config
        # When use_aie_addnorm_ffn is True, the FFN and 2 Add&Norm layers are fused together and executed in MHA block
        if not self.config.aie_config.use_aie_addnorm_ffn:
            if config.aie_config.use_aie_ffn:
                aie_ffn_config = {
                    "b_col_maj": False,
                    "c_col_maj": False,
                    "emulate_bf16_mmul_with_bfp16": True,
                    "n_a_tiles_distributed": 8 if seq_len > 512 else 4,
                    "n_b_tiles_distributed": 2 if seq_len > 512 else 4,
                    "stage_only": None,
                    "gelu_stage": 1,
                ...
                self.ffn = AIEFFN(
                    ...
                    tile_m=64,
                    tile_k=96 if seq_len > 512 else 48,
                    tile_n=48 if seq_len > 512 else 96,
                    down_proj_depth=8,
                    num_aie_columns=8,
                    **aie_ffn_config,
                )
                ...
                if config.aie_config.use_aie_gelu:
                    gelu_tile_size = (
                        seq_len * config.model_config.intermediate_size
                    ) // 16
                    self.gelu = AIEGELU(
