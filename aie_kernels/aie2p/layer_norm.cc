...
void layer_norm(const T *restrict input, T *restrict output, int32_t cols, int32_t rows_to_process)
{
    ...
    for (int row = 0; row < rows_to_process; row++) {
        ...
        input_index -= cols; // reset for next calculations
        ...
        for (int i = 0; i < vector_chunks; i++) {
            ...
            // ::aie::store_v(output + input_index, out_v);
...
void layer_norm_bf16_f32_calculation(const bfloat16 *restrict input, bfloat16 *restrict output, int32_t cols)
{
    ...
    // Note: Using a vector of 8 for float dtype because using a size of 16 causes a compile
    // error with llvm-aie (peano)
...
extern "C" {
void layer_norm(bfloat16 *input, bfloat16 *output, int32_t cols, int32_t rows_to_process)
{
    ::aie::set_rounding(aie::rounding_mode::conv_even);
    layer_norm<bfloat16, 16>(input, output, cols, rows_to_process);
}
void eltwise_mul_bf16_vector(bfloat16 *a_in, bfloat16 *b_in, bfloat16 *c_out, int cols, int32_t rows_to_process)
{
    ::aie::set_rounding(aie::rounding_mode::conv_even);
    eltwise_vmul<bfloat16, bfloat16>(a_in, b_in, c_out, cols, rows_to_process);
}
}
