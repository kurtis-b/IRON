...
Based on the performance of the isolated stages, i.e. max of last two vs max of the 3rd and 4th points, GeLU being in the down projection stage is better. This warrants some thought as to why it happens--maybe due to the different data movement pattern, and how tiles across the output columns are stored in MTs for down projection? Or maybe the AIE kernel became more efficient by having the accumulation load and output stores use separate buffers in the down projection? 
