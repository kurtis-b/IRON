...
def generate_golden_reference(
    M: int, K: int, m: int, k: int, t: int, debug_idxs=False, dtype="bf16", seed=42
):
    ...
    if debug_idxs:
        # Force output tensor to be indexes of the array, use int64 to avoid hitting
        # bf16 range limits
        output_tensor = torch.arange(M * K, dtype=torch.int64).reshape(M, K)
    ...
    # tensor into contiguous tiles of (m x k), where within each tile the data is laid out in (m x t) subtiles.
    output_rearranged = torch.empty_like(output_tensor)
    num_m_tiles = M // m
    num_k_tiles = K // k
    for m_tile in range(num_m_tiles):
        for k_tile in range(num_k_tiles):
            for k_subtile in range(k // t):
                for m_sub in range(m):
                    for t_idx in range(t):
                        src_row = m_tile * m + m_sub
                        src_col = k_tile * k + k_subtile * t + t_idx
                        dst_index = (
    ...
    if debug_idxs:
        ...
        with open(output_file, "w") as f:
            f.write(f"M={M}, K={K}, m={m}, k={k}, t={t}, dtype={dtype}\n")
