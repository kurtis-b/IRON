...
from .cases import EXECUTION_MODES, FAMILY_IDS, MODE_OPERATORS, SEQUENCE_LADDER
...
def _candidate_count_summary(execution_mode: str) -> dict[str, dict[str, int]]:
    payload = json.loads(
        ...
        rows.append(
            {
                ...
                "operator_candidate_count_summary_json": json.dumps(
                    _candidate_count_summary(execution_mode),
                    sort_keys=True,
                ),
...
def main(argv: list[str] | None = None) -> int:
    ...
    LOGGER.info("Wrote %d fairness rows to %s", len(rows), args.output)
    return 0
