        ...
        for group in groups:
            if not _selected_group(
                ...
                existing_group_rows = [
                    row
                    for row in existing_row_list
                    if str(row.get("workload_variant") or "") == group.workload_variant
                    and str(row.get("study_case_id") or "") == group.study_case_id
                    and int(float(str(row.get("seq_len") or 0))) == int(group.seq_len)
                ]
                updated_rows.extend(existing_group_rows)
                continue
            updated_rows.extend(
                build_power_only_rows_for_group(
                    group=group,
