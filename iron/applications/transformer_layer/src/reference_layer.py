...
class ReferenceTransformerLayer(nn.Module):
    ...
    def forward(
        ...
        batch, _, seq_len, _ = layer_inputs.q.shape
        hidden = self.spec.hidden_size
        head_dim = self.spec.attention_head_size
        attn_scores = torch.matmul(layer_inputs.q, layer_inputs.k.transpose(-1, -2))
        attn_scores = attn_scores * (head_dim**-0.5)
        attn_probs = torch.softmax(attn_scores.to(torch.float32), dim=-1).to(
            layer_inputs.q.dtype
        )
        attn_context = torch.matmul(attn_probs, layer_inputs.v)
        attn_context = (
            attn_context.transpose(1, 2).contiguous().view(batch, seq_len, hidden)
        )

        attention_output = self.out_proj(attn_context)
        attention_output = self.ln1(attention_output + layer_inputs.r)
