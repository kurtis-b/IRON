...
from ..bench.npu_inference import (
    INFERENCE_END_POINT,
...
def _ensure_finite_output(
    ...
    if torch.isfinite(output).all():
        return
    raise RuntimeError(
        "runlist produced non-finite output on the current pattern surface "
        f"(mode={mode}, seq_len={spec.seq_len}, "
...
def benchmark_operator_runlist_request(request: dict[str, object]) -> dict[str, object]:
    ...
    try:
        ...
        estimated_flops = estimate_layer_flops(spec, execution_mode="runlist")
        estimated_bytes = estimate_layer_bytes(spec, execution_mode="runlist")
        ...
        if context is not None and hasattr(context, "reset_runtime"):
            try:
...
def run_worker(request: dict[str, object]) -> dict[str, object]:
    mode = str(request.get("mode", "benchmark"))
    if mode == "benchmark":
        return benchmark_operator_runlist_request(request)
    if mode == "parity":
        return parity_operator_runlist_request(request)
    raise ValueError(f"Unsupported runlist worker mode: {mode}")
