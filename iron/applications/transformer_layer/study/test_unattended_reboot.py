...
def _write_measured_csv(path: Path, statuses, failure_message=""):
    path.parent.mkdir(parents=True, exist_ok=True)
    with path.open("w", newline="", encoding="utf-8") as handle:
        writer = csv.DictWriter(handle, ("seq_len", "run_status", "failure_message"))
        writer.writeheader()
            ...
            writer.writerow(
                {
                    "seq_len": 512 + index,
                    "run_status": status,
                    "failure_message": "" if status == "passed" else failure_message,
                }
            )


def test_measured_row_failures_flags_a_csv_with_no_passed_row(tmp_path):
    """A smoke run whose measurements all failed still writes complete CSVs.

    Verifying only that files exist reports that as success, which defeats the
    point of running a smoke test before a multi-day suite.
    """
    _write_measured_csv(
        tmp_path / "end_to_end" / "results_all_power.csv",
        ["failed_exception"] * 3,
        failure_message="CalledProcessError: turbostat returned non-zero exit status 2.",
    )

    problems = _measured_row_failures(tmp_path)

    assert len(problems) == 1
    assert "results_all_power.csv" in problems[0]
    assert "none with run_status=passed" in problems[0]
    assert "turbostat" in problems[0]


def test_measured_row_failures_accepts_a_partially_passing_csv(tmp_path):
    _write_measured_csv(
        tmp_path / "end_to_end" / "results_all_power.csv",
        ["passed", "failed_exception"],
        failure_message="boom",
    )

    assert _measured_row_failures(tmp_path) == []


def test_measured_row_failures_ignores_absent_files(tmp_path):
    assert _measured_row_failures(tmp_path) == []
...
def test_clear_ttm_reboot_check_allows_kernel_change_normal_drift(monkeypatch):
    """A reboot may land on a different kernel, whose default pages limit differs.

    Observed drift was 602 pages out of ~3.99M. With an absolute one-page
    tolerance clear_ttm stayed unsatisfied after the reboot, so the queue halted
    rather than reboot-loop.
    """
    monkeypatch.setattr(
        "iron.applications.transformer_layer.study.unattended_reboot._current_ttm_pages_limit",
        lambda: 3993435,
    ...
    assert (
        _ttm_action_requires_reboot(
            {"action": "clear_ttm"},
            {"normal_ttm_pages_limit": 3992833},
        )
        is False
    )
