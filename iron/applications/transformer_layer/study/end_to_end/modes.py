...
# setup jitter and background package noise.
DEFAULT_MIN_POWER_SAMPLE_COUNT = 16
DEFAULT_POWER_SAMPLE_TARGET_COUNT = 20
DEFAULT_MIN_POWER_MEASUREMENT_DURATION_SEC = max(
    2.0,
    DEFAULT_POWER_SAMPLE_INTERVAL_SEC * DEFAULT_POWER_SAMPLE_TARGET_COUNT,
)
SHORT_SEQ_POWER_PROBE_MAX_SEQ_LEN = 256
SHORT_SEQ_MIN_POWER_SAMPLE_COUNT = 20
SHORT_SEQ_POWER_SAMPLE_TARGET_COUNT = 24
SHORT_SEQ_MIN_POWER_MEASUREMENT_DURATION_SEC = max(
    2.5,
    DEFAULT_POWER_SAMPLE_INTERVAL_SEC * SHORT_SEQ_POWER_SAMPLE_TARGET_COUNT,
)
LONG_SEQ_POWER_PROBE_MIN_SEQ_LEN = 4096
LONG_SEQ_MIN_POWER_SAMPLE_COUNT = 20
LONG_SEQ_POWER_SAMPLE_TARGET_COUNT = 24
LONG_SEQ_MIN_POWER_MEASUREMENT_DURATION_SEC = max(
    3.0,
    DEFAULT_POWER_SAMPLE_INTERVAL_SEC * LONG_SEQ_POWER_SAMPLE_TARGET_COUNT,
)
...
def resolve_power_probe_policy(seq_len: int) -> tuple[float, int, int]:
    if seq_len <= SHORT_SEQ_POWER_PROBE_MAX_SEQ_LEN:
        return (
            SHORT_SEQ_MIN_POWER_MEASUREMENT_DURATION_SEC,
            SHORT_SEQ_MIN_POWER_SAMPLE_COUNT,
            SHORT_SEQ_POWER_SAMPLE_TARGET_COUNT,
        )
    if seq_len >= LONG_SEQ_POWER_PROBE_MIN_SEQ_LEN:
        return (
            LONG_SEQ_MIN_POWER_MEASUREMENT_DURATION_SEC,
            LONG_SEQ_MIN_POWER_SAMPLE_COUNT,
            LONG_SEQ_POWER_SAMPLE_TARGET_COUNT,
        )
    return (
        DEFAULT_MIN_POWER_MEASUREMENT_DURATION_SEC,
        DEFAULT_MIN_POWER_SAMPLE_COUNT,
        DEFAULT_POWER_SAMPLE_TARGET_COUNT,
    )
...
def _new_benchmark_context(scope: str) -> AIEContext:
    ...
    context.build_dir = (
        Path.cwd() / "build" / "transformer_layer_end_to_end" / sanitized_scope
    )
    return context
...
def _measure_power(
    ...
    *,
    ...
    ) = resolve_power_probe_policy(seq_len)
        ...
        with create_power_monitor(
            ...
            while True:
                ...
                if power_probe_is_complete(
                    ...
                    min_sample_count=min_power_sample_count,
                ):
                    break
