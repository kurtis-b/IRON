...
Paper terminology maps those repo modes onto three broad data patterns:

| Paper pattern | Repo evidence | Interpretation |
| --- | --- | --- |
...
| `dataflow` | dataflow block studies plus `hybrid` selected blocks | Fused/staged NPU subgraphs carry bandwidth-heavy regions. |

The proposed `hybrid` mode is the runlist+dataflow implementation: it uses
runtime sequencing around fused/staged dataflow blocks rather than claiming a
...
time the selected operator configs and full patterns without searching the full
candidate matrix.
...
inputs used by the hybrid, runlist, and offload stacked latency plots. It reads
`results_all_power.csv`, writes detailed component timings to
...
GEMM kernels. Hybrid rows reuse the selected fused/staged dataflow block
timings from tuning and compare their sum to the full runlist+dataflow pattern.
