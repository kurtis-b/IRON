...
def load_plot_rows(
    ...
    *,
    ...
    # Index-aligned boolean Series, not plain lists: an empty list would make
    # pandas read df[mask] as an empty *column* selection, dropping every
    # column. That happens whenever no row matches execution_mode, which is
    # normal for a sparse results tree such as a smoke-test run.
    full_pattern_mask = pd.Series(
        [
