...
When the canonical `results/` tree is absent, the runner falls back to a local
`results_final/` tree and then to `results_commit*` snapshots for the end-to-end
CSVs. Neither snapshot tree ships with the repository; they are only used if you
have created them locally. There is no snapshot fallback for memcpy bandwidth;
that CSV must exist explicitly.
...
using the best measured full-path throughput row across the retained
sequence ladder.
...
Tile-count and implementation-byte attribution come from the selected-op CSVs
exported by `study/resource_usage`.
...
```bash
python3 -m iron.applications.transformer_layer.study.roofline.run
```
