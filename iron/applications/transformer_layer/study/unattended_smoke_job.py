...
REQUIRED_PLOT_FILES = (
    ...
    ("end_to_end", "offload_breakdown.svg"),
    ("end_to_end", "error_histograms.svg"),
...
def _required_output_files_for_profile(
    artifact_profile: str,
    *,
    execution_smoke: bool,
) -> tuple[tuple[str, str], ...]:
    if artifact_profile == "paper":
        return REQUIRED_PAPER_OUTPUT_FILES
    if execution_smoke:
        return REQUIRED_EXECUTION_OUTPUT_FILES
    return REQUIRED_RESULTS_FILES
...
def _verify_outputs(
    ...
    *,
    ...
    missing = []
    for parts in _required_output_files_for_profile(
        artifact_profile,
    ...
    if missing:
        raise FileNotFoundError(
            "Smoke verification did not produce expected outputs:\n"
            + "\n".join(missing)
        )
    validate_results_root(results_root, artifact_profile=artifact_profile)
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    verify.add_argument("--artifact-profile", choices=ARTIFACT_PROFILES, default="p0")
    ...
    verify_execution.add_argument(
        "--artifact-profile", choices=ARTIFACT_PROFILES, default="p0"
    )
