...
# The subset of the execution outputs that record measurements, so a row with
# run_status != passed means the NPU path did not actually work. The derived
# exports (resource_usage, roofline, selected_component_*) are excluded: they
# re-derive from these and carry their own status conventions.
MEASURED_EXECUTION_OUTPUT_FILES = (
    ("block", "results.csv"),
    ("end_to_end", "results_all_power.csv"),
...
def _measured_row_failures(results_root: Path) -> list[str]:
    """Report measurement CSVs that produced no passed row.

    Existence is not enough. A run whose every measurement failed still writes
    complete, non-empty CSVs -- one failed row per attempt, with the reason in
    `failure_message`. Reporting that as a pass defeats the purpose of a smoke
    test, which is to prove the environment works before a multi-day suite.
    """
    problems = []
    for parts in MEASURED_EXECUTION_OUTPUT_FILES:
        path = results_root.joinpath(*parts)
        if not path.exists():
            continue
        with path.open(newline="", encoding="utf-8") as handle:
            rows = list(csv.DictReader(handle))
        if not rows or "run_status" not in (rows[0] if rows else {}):
            continue
        passed = [row for row in rows if str(row.get("run_status")) == "passed"]
        if passed:
            continue
        reasons = {
            str(row.get("failure_message", "")).strip()
            for row in rows
            if str(row.get("failure_message", "")).strip()
        }
        detail = f" First failure: {sorted(reasons)[0]}" if reasons else ""
        problems.append(
            f"{path}: {len(rows)} row(s), none with run_status=passed.{detail}"
        )
    return problems
...
def _verify_execution_outputs(results_root: Path) -> None:
    ...
    failures = _measured_row_failures(results_root)
    if failures:
        raise RuntimeError(
