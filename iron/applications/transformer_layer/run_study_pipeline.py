...
RESULTS_ROOT = REPO_ROOT / "iron/applications/transformer_layer/results"
AUTOMATED_BENCHMARK = SCRIPT_DIR / "automated_benchmark.py"
BOTTLENECK_ANALYSIS = SCRIPT_DIR / "analyze_design_pattern_bottlenecks.py"
GPU_COMPARE = SCRIPT_DIR / "gpu_compare_best_npu.py"
OPERATOR_RUNLIST_COMPONENT_STUDY = SCRIPT_DIR / "operator_runlist_component_study.py"
PLOT_RESULTS = SCRIPT_DIR / "plot_design_pattern_results.py"
BENCHMARK_STEP_KINDS = {"npu_study", "component_study", "gpu_compare"}
...
def _rewrite_component_study_config(
    *,
    config_path: Path,
    output_root: Path | None,
    smoke: bool,
    ...
    if config.get("output_csv") is not None:
        if output_root is None:
            config["output_csv"] = resolve_path(config_path, str(config["output_csv"]))
        else:
            config["output_csv"] = _rewrite_output_path(
                str(config["output_csv"]),
                output_root,
            )
    if smoke and config.get("seq_lens"):
        config["seq_lens"] = [int(config["seq_lens"][0])]
        config["repeats"] = 1
    return config
...
def _build_step_command(
    *,
    ...
    if kind == "npu_study":
        ...
        return command
    if kind == "component_study":
        config_path = Path(str(step["config"]))
        patched_config = _rewrite_component_study_config(
        ...
        config_arg = _write_temp_json(
            temp_dir,
            f"{step['step_id']}_component.json",
            patched_config,
        )
        return [
            sys.executable,
            str(OPERATOR_RUNLIST_COMPONENT_STUDY),
            "--config",
            config_arg,
        ]
    if kind == "gpu_compare":
