...
| `operator_runlist` | Runtime lifecycle and stage-contract debugging were the hardest engineering surface | The retained study required child-process isolation for the benchmark/parity flow and several attention-path fixes before the composed runlist matched the standalone operators. The short-surface parity rows are now back in-family with the other patterns (`max_abs_diff=0.0625` at `seq_len=64`, `0.09375` at `seq_len=128`). | This pattern demonstrates staged NPU offload without the same topology coupling as `encoder_pipeline`, but it required the most runtime-management and inter-stage-contract debugging before the end-to-end story became credible. | Keep benchmark and parity execution isolated in child processes, use lifetime-aware BO reuse, preserve stage-level validation as a first-class tool, and use component-boundary checks for long-sequence correctness instead of full host materialization. | Completed on the retained short-surface parity path; long-sequence validation remains component-boundary by design. | [design_patterns_main_parity.csv](/home/cj/iron/iron/applications/transformer_layer/results/design_patterns_main_parity.csv) |
...
- explicit support-matrix reporting for mixed-status studies
- structured debug logging
- bottleneck summaries
- parity, component-boundary validation, and roofline post-processing
