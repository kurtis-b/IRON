...
class AIEMHAOutProj(AIEOperatorBase):
    ...
    def set_up_artifacts(self):
        ...
        causal_suffix = "causal" if self.is_causal else "noncausal"

        file_name_base = (
            f"mha_out_proj_{self.num_heads}h_{self.seq_len}s_{self.d}d_"
            f"{self.q_seq_tile}q_{self.kv_seq_tile}kv_{self.emb_tile}e_"
            f"{self.parallel_seq}ps_{self.parallel_heads}ph_{self.o_proj_acc_depth}acc_"
            f"{causal_suffix}"
        )
                ...
                KernelArchiveArtifact.new(
                    ...
                    depends=[
                        KernelObjectArtifact.new(
                            f"mha_mm_{self.q_seq_tile}m_{self.d}k_{self.kv_seq_tile}n_{causal_suffix}.o",
                            extra_flags=mm_defines_colmaj,
                            depends=[SourceArtifact.new(mm_source)],
                        ),
                        KernelObjectArtifact.new(
                            f"mha_mm_rowmaj_{self.q_seq_tile}m_{self.kv_seq_tile}k_{self.d}n_{causal_suffix}.o",
                            extra_flags=mm_defines_rowmaj,
                            depends=[SourceArtifact.new(mm_source)],
                            rename_symbols=mm_rename_symbols,
                        ),
                        KernelObjectArtifact.new(
                            f"mha_mm_o_proj_{self.q_seq_tile}m_{self.d}k_{self.emb_tile}n_{causal_suffix}.o",
                            extra_flags=mm_o_proj_defines,
                            depends=[SourceArtifact.new(mm_source)],
                            rename_symbols=mm_o_proj_rename_symbols,
                        ),
                        KernelObjectArtifact.new(
                            f"mha_softmax_{self.q_seq_tile}m_{self.kv_seq_tile}n_{causal_suffix}.o",
                            depends=[SourceArtifact.new(softmax_source)],
                            extra_flags=[f"-DSM_VEC_LEN={self.kv_seq_tile}"],
                        ),
                        KernelObjectArtifact.new(
                            f"mha_{self.q_seq_tile}m_{self.d}k_{self.kv_seq_tile}n_{causal_suffix}.o",
                            depends=[SourceArtifact.new(mha_source)],
                            extra_flags=[
                                f"-DIS_CAUSAL={1 if self.is_causal else 0}",
                                "-DDEBUG=0",
                                f"-DB_Q_TILE={self.q_seq_tile}",
                                f"-DB_KV_TILE={self.kv_seq_tile}",
                                f"-DVECTOR_LENGTH={min(self.q_seq_tile, self.kv_seq_tile)}",
                                f"-DSCALE_VECTOR_LENGTH={self.q_seq_tile}",
                            ],
                        ),
                        KernelObjectArtifact.new(
                            f"mha_pass_through_{self.q_seq_tile}m_{self.emb_tile}n_{causal_suffix}.o",
                            extra_flags=["-DBIT_WIDTH=16"],
                            depends=[SourceArtifact.new(passthrough_source)],
                        ),
                        KernelObjectArtifact.new(
                            f"mha_pass_through_o_proj_{self.q_seq_tile}m_{self.emb_tile}n_{causal_suffix}.o",
                            extra_flags=["-DBIT_WIDTH=16"],
                            depends=[SourceArtifact.new(passthrough_source)],
                            rename_symbols={
                        ...
                        KernelObjectArtifact.new(
                            f"mha_add_{self.q_seq_tile}m_{self.emb_tile}n_{causal_suffix}.o",
