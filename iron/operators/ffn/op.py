...
class AIEFFN(AIEOperatorBase):
    ...
    def _needs_runtime_reload_between_launches(self):
        stage_only = self.ffn_args.get("stage_only", None)
        return self.down_proj_depth > 1 and stage_only in (None, 1)

    def _reload_runtime_preserving_buffers(self):
        snapshots = {}
        for buffer_name, buffer_size in self.buffers.items():
            if (
                buffer_name in self.buffer_static_data
                or buffer_name not in self.buffer_bos
            ):
                continue
            mv = self.buffer_bos[buffer_name].map()
            snapshots[buffer_name] = bytes(memoryview(mv)[:buffer_size])

        self.context.reset_runtime()
        self.context.prepare_runtime(describe_runtime=False)

        for buffer_name, data in snapshots.items():
            self.write_buffer(buffer_name, np.frombuffer(data, dtype=np.uint8))
    ...
    def run_runlist(self):
        if self._launch_count > 0 and self._needs_runtime_reload_between_launches():
            self._reload_runtime_preserving_buffers()

        elapsed = super().run_runlist()
