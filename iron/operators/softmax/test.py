...
def test_row_chunk_size_tracks_long_softmax_rows():
    assert AIESoftmax._resolve_row_chunk_size(8192) == 8192
    assert AIESoftmax._resolve_row_chunk_size(16384) == 8192
    with pytest.raises(ValueError):
        AIESoftmax._resolve_row_chunk_size(12288)
...
def test_softmax_16384wide_path(aie_context):
    rows = 16
    cols = 16384
    golden_ref = generate_golden_reference(rows=rows, cols=cols)
    operator = AIESoftmax(
