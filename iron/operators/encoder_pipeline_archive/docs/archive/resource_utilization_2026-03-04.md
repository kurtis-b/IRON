...
- Same-tile full-row staging now uses a double-buffered memtile row-store where legal:
...
- Memtile channel assignment for row-store sites; BD pressure is reduced, but channel pressure is still topology-dependent
...
- The broader long-sequence complex subset is currently passing with the narrowed row-store envelope:
...
- In the `64embtile / 12pacc` row-store experiments, the first remaining allocator failure is now the final `memLN2` shim drain, not LN/O-proj/FFN-down row staging. A direct row-major `memLN2` drain probe did not change that outcome, so the remaining limit is the runtime output tap count on the final shim drain rather than the `dimensionsToStream` form of `memLN2`.
